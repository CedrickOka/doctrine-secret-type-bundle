...
namespace Oka\Doctrine\SecretTypeBundle\Command;
...
final class GenerateKeyPairCommand extends Command
{
    ...
    protected function configure(): void
    {
        ...
        $this->addOption('algorithm', null, InputOption::VALUE_REQUIRED, 'The algorithm signature.', 'RS256');
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        if (false === in_array($algorithm, self::ACCEPTED_ALGORITHMS, true)) {
        ...
        if (true === $input->getOption('dry-run')) {
            ...
            $io->newLine();
            $io->writeln(sprintf('Update your private key in <info>%s</info>:', $this->privateKeyFile));
            $io->writeln($privateKey);
            $io->newLine();
            $io->writeln(sprintf('Update your public key in <info>%s</info>:', $this->publicKeyFile));
        ...
        if (!$this->privateKeyFile || !$this->publicKeyFile) {
            throw new LogicException(sprintf('The "oka_doctrine_secret_type.private_key_file" and "oka_doctrine_secret_type.public_key_file" config options must not be empty for using the "%s" command.', self::$defaultName));
        }

        $filesystem = new Filesystem();

        if (true === $filesystem->exists($this->privateKeyFile) || true === $filesystem->exists($this->publicKeyFile)) {
            try {
        ...
        $filesystem->dumpFile($this->privateKeyFile, $privateKey);
        $filesystem->dumpFile($this->publicKeyFile, $publicKey);
